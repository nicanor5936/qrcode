...
let logoImage = null; // image chargée pour le logo central

// Échappe les caractères spéciaux réservés par le format Wi-Fi (; , : \)
...
function drawLogo() {
  ...
  const logoSize = canvas.width * 0.22; // 22% de la taille du QR
  const x = (canvas.width - logoSize) / 2;
  const y = (canvas.height - logoSize) / 2;

  // fond blanc/couleur de fond derrière le logo pour garder le QR lisible
  const pad = 6;
...
logoInput.addEventListener('change', () => {
  ...
  img.onload = () => {
    logoImage = img;
    // Un logo cache une partie du QR code : on force une correction d'erreur
    // elevee pour qu'il reste scannable.
    if (eccEl.value !== 'H') {
      eccEl.value = 'H';
...
// Recharge un element d'historique dans le formulaire correspondant
...
// Recupere les valeurs actuelles des champs du mode courant, pour les sauvegarder
function getCurrentFieldValues() {
  const idsByMode = {
    website: ['website-url'],
    whatsapp: ['wa-number', 'wa-message'],
    facebook: ['fb-username'],
    instagram: ['ig-username'],
    tiktok: ['tt-username'],
    youtube: ['yt-channel'],
    linkedin: ['li-username'],
    x: ['x-username'],
    snapchat: ['sc-username'],
...
async function loadHistory() {
  const items = await ipcRenderer.invoke('history-get');
  renderHistory(items);
}

btnClearHistory.addEventListener('click', async () => {
  const items = await ipcRenderer.invoke('history-clear');
  renderHistory(items);
});
...
// Construit une entree d'historique lisible a partir de l'etat actuel
...
async function saveToHistory() {
  ...
  const items = await ipcRenderer.invoke('history-add', entry);
  renderHistory(items);
}
...
document.getElementById('btn-download').addEventListener('click', async () => {
  ...
  const result = await ipcRenderer.invoke('save-png', dataUrl, 'qrcode.png');
  if (result.ok) {
    statusEl.textContent = `Enregistré : ${result.filePath}`;
...
document.getElementById('btn-copy').addEventListener('click', async () => {
  canvas.toBlob(async (blob) => {
    try {
      ...
      statusEl.textContent = 'Copie impossible sur ce système.';
      statusEl.className = 'status';
    }
  });
});
